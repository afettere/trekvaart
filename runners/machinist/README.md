...
3b. The wrapper asks the sweeper what an exit means. An exit 0 with the issue at `trekvaart:verifying` and the foreman's PR comment posted is a turn that ended inside the automation gate (the first product flight, 2026-09-09, left two minutes into a 50-minute CI wait); the wrapper runs the foreman again with the same prompt, up to three times, each resume priced as its own run (`<run id>-r1`, `-r2`, `-r3`). Any other exit with the issue in flight is a strand: `needs-human` with a stop comment.
